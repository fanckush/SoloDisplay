  ...
  public static func read(validation: BackendValidation? = nil) -> PlatformReading {
    ...
    if root != 0 {
      ...
      if let value = IORegistryEntryCreateCFProperty(
        root, "AppleClamshellState" as CFString, nil, 0)?.takeRetainedValue()
      {
        if CFGetTypeID(value) == CFBooleanGetTypeID() {
          lid = CFBooleanGetValue((value as! CFBoolean)) ? .closed : .open
        }
      } else {
    ...
    if displays.contains(where: \.mirrored) {
      limitations.insert(
        "Mirroring detected. Mirroring plus a dimmed built-in screen is a normal incoming setup. Preserving its arrangement through suppression and restoration still needs a separate hardware experiment; Lidless will not silently convert it to extended mode.",
        at: 0)
    }
