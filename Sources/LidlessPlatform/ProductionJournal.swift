...
public final class ProductionJournalStore: Sendable {
    ...
    -> JournalReconciliation
  {
    ...
    do { record = try load() } catch {
      return .retained(
        "Lidless found an unreadable recovery record and will not turn the internal display off until it is resolved. Export diagnostics or remove the file it names."
      )
    }
    guard let record else { return .clean }
    do { try record.validate() } catch {
      return .retained(
        "Lidless found a recovery record it does not understand and will not turn the internal display off until it is resolved."
      )
    }
    guard let bootID, let loginID else {
      return .retained(
        "Lidless cannot read this Mac's startup or login session identity, so it cannot resolve the recovery record it found."
      )
    }
    ...
    do { try RecoveryIdentity.checkCurrentDisplays(displays, target: record.target) } catch {
      return .retained(
        "Live display evidence contradicts Lidless's recovery record, so no display was changed. Turning the internal display off stays unavailable."
      )
    }
