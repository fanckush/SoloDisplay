...
func probe(_ flags: [String: String]) throws {
  ...
  guard flags["--native-wired-attested"] == "true",
    let external = UInt32(try required("--external", from: flags))
  else {
  ...
  let journalURL = URL(fileURLWithPath: try required("--journal", from: flags))
...
func verifyRestored(_ displayID: UInt32) throws {
  ...
  repeat {
    if CGDisplayIsBuiltin(displayID) != 0 && CGDisplayIsActive(displayID) != 0 { return }
    RunLoop.current.run(until: Date(timeIntervalSinceNow: 0.1))
  } while ProcessInfo.processInfo.systemUptime < deadline
  throw LabError.message(
...
do {
  ...
  case "replay":
    ...
    guard trace.events.count <= 10_000 else {
  ...
  case "example-trace":
    ...
    try recorder.append(
      .init(
        at: 2_000, event: .observed(.init(sequence: 2, sampledAt: 2_000, environment: environment)))
    )
    try recorder.append(.init(at: 2_001, event: .journalSaved(operationID: 1, succeeded: true)))
    try recorder.append(
      .init(at: 2_010, event: .operationReturned(operationID: 1, succeeded: true)))
    environment.panelState = .disabled
    try recorder.append(
      .init(
        at: 2_020, event: .observed(.init(sequence: 3, sampledAt: 2_020, environment: environment)))
    )
    environment.nativeExternalAvailable = .no
    try recorder.append(
      .init(
        at: 3_000, event: .observed(.init(sequence: 4, sampledAt: 3_000, environment: environment)))
    )
    try recorder.append(
      .init(at: 3_010, event: .operationReturned(operationID: 2, succeeded: true)))
    environment.panelState = .enabled
    try recorder.append(
      .init(
        at: 3_020, event: .observed(.init(sequence: 5, sampledAt: 3_020, environment: environment)))
    )
    print(String(decoding: try recorder.exportSanitized(), as: UTF8.self))
