...
final class SoloDisplayUITests: XCTestCase {
  @MainActor
  func testDiagnosticsAreExplicitlyReadOnly() {
    ...
    // A UI test bundle does not link the app, so this literal has to track ProductionLaunch.
    app.launchArguments = ["--solodisplay-unprotected", "--diagnostics"]
    app.launch()
