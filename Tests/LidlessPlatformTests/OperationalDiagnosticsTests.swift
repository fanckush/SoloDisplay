...
private struct HistoryStub: OperationalHistoryReading {
  ...
  func read(from: Date, through: Date) -> OperationalHistory { history }
}

private struct FailingDiagnosticsWriter: DiagnosticsFileWriting {
  func write(_ data: Data, to url: URL) throws { throw CocoaError(.fileWriteNoPermission) }
}

struct OperationalDiagnosticsTests {
  @Test func typedSinkRejectsUnsafeIdentifiersAndBuildStrings() throws {
  ...
  @Test func maximalOperationalRecordFitsUnifiedLogPayload() throws {
    ...
    logger.emit(
      ...
      $0.challengeAgeMS = 5_000
      $0.challenge = 99
      $0.leaseDeadlineMS = 1_000_000
    }
    #expect(try JSONEncoder().encode(sink.events[0]).count <= 1_000)
  }
  ...
  @Test func exportsFilterRawTextAndPreserveExportLocalPairing() throws {
    ...
    let controller = UUID()
    let session = UUID()
    var a = OperationalEvent(code: .paired, role: .helper, run: helper, uptimeMS: 1)
    var b = OperationalEvent(code: .paired, role: .controller, run: controller, uptimeMS: 2)
    a.session = session.uuidString
    b.session = session.uuidString
    var raw = try #require(
      JSONSerialization.jsonObject(with: JSONEncoder().encode(a)) as? [String: Any])
    raw["secret"] = "PRIVATE-display-serial"
    a = try JSONDecoder().decode(
    ...
    let data = try DiagnosticsExporter.encode(
      ...
      history: .init(
        status: .collected,
        entries: [
          .init(date: now, event: a), .init(date: now, event: b), .init(date: now, event: invalid),
  ...
  @Test func combinedExportEvictsOldestEventsAndAdvancesReplayBaseline() throws {
    ...
    let history = OperationalHistory(
      ...
      entries: (1...2_200).map {
        .init(
          date: now.addingTimeInterval(Double($0 - 2_200)),
          event: .init(code: .started, role: .helper, run: run, uptimeMS: Int64($0)))
      })
    let data = try DiagnosticsExporter.encode(
      trace: trace, history: history,
      from: now.addingTimeInterval(-86_400), through: now, collectedAt: now)
    let document = try JSONDecoder().decode(DiagnosticsDocument.self, from: data)
    let replay = try JSONDecoder().decode(ReplayTrace.self, from: data)
    #expect(data.count <= 5_000_000)
    #expect(document.events.count + document.diagnostics.history.entries.count <= 10_000)
    #expect(document.diagnostics.history.entries.count <= 2_000)
    ...
    #expect(document.diagnostics.history.entries.last?.event.uptimeMS == 2_200)
  }
