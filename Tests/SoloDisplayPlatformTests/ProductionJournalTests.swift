...
private func workspace() throws -> URL {
  let url = FileManager.default.temporaryDirectory
    .appendingPathComponent("solodisplay-journal-\(UUID().uuidString)", isDirectory: true)
...
struct ProductionJournalTests {
  ...
  @Test func theSupportDirectoryMovesWholeWhenNothingIsThereYet() throws {
    let root = try workspace()
    let legacy = root.appendingPathComponent("Lidless", isDirectory: true)
    let current = root.appendingPathComponent("SoloDisplay", isDirectory: true)
    let store = try ProductionJournalStore(directory: legacy)
    try store.prepare(record())
    try Data("{}".utf8).write(to: legacy.appendingPathComponent("preferences.json"))

    try ProductionJournalStore.migrateLegacyDirectory(from: legacy, to: current)

    #expect(!FileManager.default.fileExists(atPath: legacy.path))
    #expect(try ProductionJournalStore(directory: current).load()?.operationID == 1)
    #expect(
      FileManager.default
        .fileExists(atPath: current.appendingPathComponent("preferences.json").path)
    )
  ...
  /// A destination that already exists was written by the current build and outranks anything
  /// left behind, so migration may only fill gaps.
  @Test func anExistingRecordIsNeverOverwrittenByTheLegacyOne() throws {
    let root = try workspace()
    let legacy = root.appendingPathComponent("Lidless", isDirectory: true)
    let current = root.appendingPathComponent("SoloDisplay", isDirectory: true)
    try ProductionJournalStore(directory: legacy).prepare(record(operationID: 1))
    try Data("{}".utf8).write(to: legacy.appendingPathComponent("preferences.json"))
    try ProductionJournalStore(directory: current).prepare(record(operationID: 2))

    try ProductionJournalStore.migrateLegacyDirectory(from: legacy, to: current)

    #expect(try ProductionJournalStore(directory: current).load()?.operationID == 2)
    // The gap is filled, and the record that could not move is kept as evidence.
    #expect(
      FileManager.default
        .fileExists(atPath: current.appendingPathComponent("preferences.json").path)
    )
    #expect(FileManager.default
      .fileExists(atPath: legacy.appendingPathComponent("recovery.json").path))
  }

  @Test func migrationIsInertWhenThereIsNothingToMoveOrNowhereToMoveIt() throws {
    let root = try workspace()
    let legacy = root.appendingPathComponent("Lidless", isDirectory: true)
    let current = root.appendingPathComponent("SoloDisplay", isDirectory: true)

    // A first run on a clean Mac.
    #expect(throws: Never.self) {
      try ProductionJournalStore.migrateLegacyDirectory(from: legacy, to: current)
    }
    #expect(!FileManager.default.fileExists(atPath: current.path))

    // Before the rename lands, both names resolve to the same directory.
    try ProductionJournalStore(directory: legacy).prepare(record())
    #expect(throws: Never.self) {
      try ProductionJournalStore.migrateLegacyDirectory(from: legacy, to: legacy)
    }
    #expect(try ProductionJournalStore(directory: legacy).load()?.operationID == 1)

    // A concurrent launch that already moved everything leaves nothing to do.
    try ProductionJournalStore.migrateLegacyDirectory(from: legacy, to: current)
    #expect(throws: Never.self) {
      try ProductionJournalStore.migrateLegacyDirectory(from: legacy, to: current)
    }
