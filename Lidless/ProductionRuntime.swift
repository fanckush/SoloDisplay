...
/// The menu-bar process. It pairs with its helper and holds exclusive writer ownership for the
/// whole run. It cannot disable anything yet: the production coordinator is a later milestone.
@MainActor
final class ControllerRuntime {
  ...
  private var timer: Timer?
  private(set) var availability: ProductionAvailability

  init(link: ProtectionLink) {
    self.link = link
    let session = UUID().uuidString
    protection = .init(session: session, at: HelperRuntime.now())
    availability = .unavailable(
      "Turning the internal display off is not enabled in this build yet.")
  }
  ...
  func start() {
    ...
    if writerLock == nil {
      availability = .unavailable(
        "Another display writer already owns this login session, so Lidless will not change displays."
      )
    }
    apply(protection.receive(.start, at: HelperRuntime.now()))
    timer = Timer.scheduledTimer(withTimeInterval: 0.2, repeats: true) { [weak self] _ in
      Task { @MainActor in self?.pump() }
    }
  }
  ...
  func stop() {
    timer?.invalidate()
    // Nothing was ever armed, so there is no ownership to hand back or leave behind.
    apply(protection.receive(.release, at: HelperRuntime.now()))
    link.stop()
    writerLock?.release()
  }

  private func pump() {
    let now = HelperRuntime.now()
    do {
      while let message = try link.poll() {
        apply(protection.receive(.received(message), at: now))
      }
  ...
  private func apply(_ outputs: [ControllerProtection.Output]) {
    for output in outputs {
      switch output {
      case .send(let message): try? link.send(message)
      case .protectionEstablished: availability = .protectedIdle
      case .protectionLost:
        // Milestone B restores an owned panel here. Nothing is owned in this build.
        availability = .unavailable(
          "Lidless lost contact with its recovery helper, so display control stays unavailable.")
      }
    }
  }
}
