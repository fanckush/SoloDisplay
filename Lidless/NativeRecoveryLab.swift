  ...
  final class NativeRecoveryLab {
    ...
    func report(_ label: String) throws {
      ...
      guard let output = String(bytes: data, encoding: .utf8) else {
        throw NativeLabError.refused("Failed to encode the native report as UTF-8.")
      }
      print(output)
      fflush(stdout)
    }
    ...
    private func restoreChild(path: String) async throws {
      ...
      // The live actual parent provides ownership. This does not relax ordinary after-crash
      // identity checks.
