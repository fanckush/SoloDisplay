...
A normal launch becomes the supervising helper. It owns no user interface, never disables a display, and launches its controller child from the same app executable over inherited private pipes. Both processes run real AppKit event loops. There is no daemon, no root privilege, and no background installation. A per-login instance lock refuses a second pair, and the controller holds the writer lock for its whole run, so the helper cannot acquire it while its controller lives.
...
The production coordinator, effect executor, lifecycle recovery policy, and native transport classification are not complete. See `status.md` for the current checklist instead of inferring completion from the existence of a model, a lab command, or a process role.
